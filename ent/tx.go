...
func (tx *Tx) init() {
	tx.ApplicantInterest = NewApplicantInterestClient(tx.config)
	tx.ApplicantProfile = NewApplicantProfileClient(tx.config)
	tx.ApplicantProfileSkill = NewApplicantProfileSkillClient(tx.config)
	tx.Application = NewApplicationClient(tx.config)
	tx.Category = NewCategoryClient(tx.config)
	tx.ContactUsMessage = NewContactUsMessageClient(tx.config)
	tx.DegreeLevel = NewDegreeLevelClient(tx.config)
	...
	tx.JobOfferCategory = NewJobOfferCategoryClient(tx.config)
...
// applies a query, for example: ApplicantInterest.QueryXXX(), the query will be executed

...
func (sq *SkillQuery) QueryApplicantProfileSkills() *ApplicantProfileSkillQuery {
	query := (&ApplicantProfileSkillClient{config: sq.config}).Query()
...
func (sq *SkillQuery) Clone() *SkillQuery {
	...
	return &SkillQuery{
		...
		withApplicantProfileSkills: sq.withApplicantProfileSkills.Clone(),
...
func (sq *SkillQuery) WithApplicantProfileSkills(opts ...func(*ApplicantProfileSkillQuery)) *SkillQuery {
	query := (&ApplicantProfileSkillClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withApplicantProfileSkills = query
	return sq
}
...
func (sq *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
			sq.withApplicantProfileSkills != nil,
		}
	)
	...
	if query := sq.withApplicantProfileSkills; query != nil {
		if err := sq.loadApplicantProfileSkills(ctx, query, nodes,
			func(n *Skill) { n.Edges.ApplicantProfileSkills = []*ApplicantProfileSkill{} },
			func(n *Skill, e *ApplicantProfileSkill) {
...
func (sq *SkillQuery) loadApplicantProfileSkills(ctx context.Context, query *ApplicantProfileSkillQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *ApplicantProfileSkill)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(applicantprofileskill.FieldSkillID)
	}
	query.Where(predicate.ApplicantProfileSkill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.ApplicantProfileSkillsColumn), fks...))
	}))

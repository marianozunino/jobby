...
func (lq *LanguageQuery) QueryApplicantProfile() *ApplicantProfileQuery {
	query := (&ApplicantProfileClient{config: lq.config}).Query()
...
func (lq *LanguageQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (lq *LanguageQuery) FirstIDX(ctx context.Context) uint {
...
func (lq *LanguageQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (lq *LanguageQuery) OnlyIDX(ctx context.Context) uint {
...
func (lq *LanguageQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (lq *LanguageQuery) IDsX(ctx context.Context) []uint {
	ids, err := lq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (lq *LanguageQuery) Clone() *LanguageQuery {
	...
	return &LanguageQuery{
		...
		withApplicantProfile: lq.withApplicantProfile.Clone(),
...
func (lq *LanguageQuery) WithApplicantProfile(opts ...func(*ApplicantProfileQuery)) *LanguageQuery {
	query := (&ApplicantProfileClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withApplicantProfile = query
	return lq
}
...
func (lq *LanguageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Language, error) {
	...
	if query := lq.withApplicantProfile; query != nil {
		if err := lq.loadApplicantProfile(ctx, query, nodes, nil,
			func(n *Language, e *ApplicantProfile) { n.Edges.ApplicantProfile = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LanguageQuery) loadApplicantProfile(ctx context.Context, query *ApplicantProfileQuery, nodes []*Language, init func(*Language), assign func(*Language, *ApplicantProfile)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Language)
	for i := range nodes {
		if nodes[i].ApplicantProfileID == nil {
			continue
		}
...
func (lq *LanguageQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(language.Table, language.Columns, sqlgraph.NewFieldSpec(language.FieldID, field.TypeUint))
	_spec.From = lq.sql
	...
	if fields := lq.ctx.Fields; len(fields) > 0 {
		...
		if lq.withApplicantProfile != nil {
			_spec.Node.AddColumnOnce(language.FieldApplicantProfileID)
		}
	}

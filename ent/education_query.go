...
type EducationQuery struct {
	...
	withDegreeLevel *DegreeLevelQuery
	withFKs         bool
...
// QueryDegreeLevel chains the current query on the "degree_level" edge.
func (eq *EducationQuery) QueryDegreeLevel() *DegreeLevelQuery {
	query := (&DegreeLevelClient{config: eq.config}).Query()
...
func (eq *EducationQuery) Clone() *EducationQuery {
	...
	return &EducationQuery{
		...
		withDegreeLevel: eq.withDegreeLevel.Clone(),
...
// WithDegreeLevel tells the query-builder to eager-load the nodes that are connected to
// the "degree_level" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EducationQuery) WithDegreeLevel(opts ...func(*DegreeLevelQuery)) *EducationQuery {
	query := (&DegreeLevelClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withDegreeLevel = query
	return eq
}
...
func (eq *EducationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Education, error) {
	var (
		...
		_spec       = eq.querySpec()
		loadedTypes = [1]bool{
			eq.withDegreeLevel != nil,
		}
	)
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, education.ForeignKeys...)
	}
	...
	if query := eq.withDegreeLevel; query != nil {
		if err := eq.loadDegreeLevel(ctx, query, nodes, nil,
			func(n *Education, e *DegreeLevel) { n.Edges.DegreeLevel = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EducationQuery) loadDegreeLevel(ctx context.Context, query *DegreeLevelQuery, nodes []*Education, init func(*Education), assign func(*Education, *DegreeLevel)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Education)
	for i := range nodes {
		if nodes[i].DegreeLevelID == nil {
			continue
		}
		fk := *nodes[i].DegreeLevelID
	...
	query.Where(degreelevel.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "degree_level_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EducationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withDegreeLevel != nil {
			_spec.Node.AddColumnOnce(education.FieldDegreeLevelID)
		}
	}

...
func (aq *ApplicationQuery) QueryApplicantProfile() *ApplicantProfileQuery {
	query := (&ApplicantProfileClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) QueryJobOffer() *JobOfferQuery {
	query := (&JobOfferClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ApplicationQuery) QueryInterviews() *InterviewQuery {
	query := (&InterviewClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withApplicantProfile: aq.withApplicantProfile.Clone(),
		withJobOffer:         aq.withJobOffer.Clone(),
		withInterviews:       aq.withInterviews.Clone(),
...
func (aq *ApplicationQuery) WithApplicantProfile(opts ...func(*ApplicantProfileQuery)) *ApplicationQuery {
	query := (&ApplicantProfileClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withApplicantProfile = query
	return aq
}
...
func (aq *ApplicationQuery) WithJobOffer(opts ...func(*JobOfferQuery)) *ApplicationQuery {
	query := (&JobOfferClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withJobOffer = query
	return aq
}
...
func (aq *ApplicationQuery) WithInterviews(opts ...func(*InterviewQuery)) *ApplicationQuery {
	query := (&InterviewClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withInterviews = query
	return aq
}
...
//		GroupBy(application.FieldJobOfferID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Application.Query().
//		Select(application.FieldJobOfferID).
//		Scan(ctx, &v)
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [3]bool{
			aq.withApplicantProfile != nil,
			aq.withJobOffer != nil,
			aq.withInterviews != nil,
		}
	)
	...
	if query := aq.withApplicantProfile; query != nil {
		if err := aq.loadApplicantProfile(ctx, query, nodes, nil,
			func(n *Application, e *ApplicantProfile) { n.Edges.ApplicantProfile = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withJobOffer; query != nil {
		if err := aq.loadJobOffer(ctx, query, nodes, nil,
			func(n *Application, e *JobOffer) { n.Edges.JobOffer = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withInterviews; query != nil {
		if err := aq.loadInterviews(ctx, query, nodes,
			func(n *Application) { n.Edges.Interviews = []*Interview{} },
			func(n *Application, e *Interview) { n.Edges.Interviews = append(n.Edges.Interviews, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ApplicationQuery) loadApplicantProfile(ctx context.Context, query *ApplicantProfileQuery, nodes []*Application, init func(*Application), assign func(*Application, *ApplicantProfile)) error {
...
func (aq *ApplicationQuery) loadJobOffer(ctx context.Context, query *JobOfferQuery, nodes []*Application, init func(*Application), assign func(*Application, *JobOffer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Application)
	for i := range nodes {
...
func (aq *ApplicationQuery) loadInterviews(ctx context.Context, query *InterviewQuery, nodes []*Application, init func(*Application), assign func(*Application, *Interview)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(interview.FieldApplicationID)
	}
	query.Where(predicate.Interview(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.InterviewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ApplicationID
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (aq *ApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withApplicantProfile != nil {
			_spec.Node.AddColumnOnce(application.FieldApplicantProfileID)
		}
		if aq.withJobOffer != nil {
			_spec.Node.AddColumnOnce(application.FieldJobOfferID)
		}
	}

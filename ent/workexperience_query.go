...
func (weq *WorkExperienceQuery) QueryApplicantProfile() *ApplicantProfileQuery {
	query := (&ApplicantProfileClient{config: weq.config}).Query()
...
func (weq *WorkExperienceQuery) Clone() *WorkExperienceQuery {
	...
	return &WorkExperienceQuery{
		...
		withApplicantProfile: weq.withApplicantProfile.Clone(),
...
func (weq *WorkExperienceQuery) WithApplicantProfile(opts ...func(*ApplicantProfileQuery)) *WorkExperienceQuery {
	query := (&ApplicantProfileClient{config: weq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	weq.withApplicantProfile = query
	return weq
}
...
func (weq *WorkExperienceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*WorkExperience, error) {
	...
	if query := weq.withApplicantProfile; query != nil {
		if err := weq.loadApplicantProfile(ctx, query, nodes, nil,
			func(n *WorkExperience, e *ApplicantProfile) { n.Edges.ApplicantProfile = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (weq *WorkExperienceQuery) loadApplicantProfile(ctx context.Context, query *ApplicantProfileQuery, nodes []*WorkExperience, init func(*WorkExperience), assign func(*WorkExperience, *ApplicantProfile)) error {
...
func (weq *WorkExperienceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := weq.ctx.Fields; len(fields) > 0 {
		...
		if weq.withApplicantProfile != nil {
			_spec.Node.AddColumnOnce(workexperience.FieldApplicantProfileID)
		}
	}

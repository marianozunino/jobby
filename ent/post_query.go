...
func (pq *PostQuery) QueryPostCategories() *PostCategoryQuery {
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withPostCategories: pq.withPostCategories.Clone(),
...
func (pq *PostQuery) WithPostCategories(opts ...func(*PostCategoryQuery)) *PostQuery {
	query := (&PostCategoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPostCategories = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withPostCategories; query != nil {
		if err := pq.loadPostCategories(ctx, query, nodes,
			func(n *Post) { n.Edges.PostCategories = []*PostCategory{} },
			func(n *Post, e *PostCategory) { n.Edges.PostCategories = append(n.Edges.PostCategories, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadPostCategories(ctx context.Context, query *PostCategoryQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostCategory)) error {
	...
	query.Where(predicate.PostCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.PostCategoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.post_post_categories
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_post_categories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_post_categories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

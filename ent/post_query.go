...
type PostQuery struct {
	...
	withUser         *UserQuery
	withPostCategory *PostCategoryQuery
...
// QueryPostCategory chains the current query on the "post_category" edge.
func (pq *PostQuery) QueryPostCategory() *PostCategoryQuery {
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withPostCategory: pq.withPostCategory.Clone(),
...
// WithPostCategory tells the query-builder to eager-load the nodes that are connected to
// the "post_category" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithPostCategory(opts ...func(*PostCategoryQuery)) *PostQuery {
	query := (&PostCategoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPostCategory = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withPostCategory; query != nil {
		if err := pq.loadPostCategory(ctx, query, nodes,
			func(n *Post) { n.Edges.PostCategory = []*PostCategory{} },
			func(n *Post, e *PostCategory) { n.Edges.PostCategory = append(n.Edges.PostCategory, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadPostCategory(ctx context.Context, query *PostCategoryQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostCategory)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.PostCategoryTable)
		s.Join(joinT).On(s.C(postcategory.FieldID), joinT.C(post.PostCategoryPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(post.PostCategoryPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.PostCategoryPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "post_category" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}

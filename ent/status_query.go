...
// QueryJobOffers chains the current query on the "job_offers" edge.
func (sq *StatusQuery) QueryJobOffers() *JobOfferQuery {
	query := (&JobOfferClient{config: sq.config}).Query()
...
func (sq *StatusQuery) Clone() *StatusQuery {
	...
	return &StatusQuery{
		...
		withJobOffers: sq.withJobOffers.Clone(),
...
// WithJobOffers tells the query-builder to eager-load the nodes that are connected to
// the "job_offers" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StatusQuery) WithJobOffers(opts ...func(*JobOfferQuery)) *StatusQuery {
	query := (&JobOfferClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withJobOffers = query
	return sq
}
...
func (sq *StatusQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Status, error) {
	...
	if query := sq.withJobOffers; query != nil {
		if err := sq.loadJobOffers(ctx, query, nodes,
			func(n *Status) { n.Edges.JobOffers = []*JobOffer{} },
			func(n *Status, e *JobOffer) { n.Edges.JobOffers = append(n.Edges.JobOffers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StatusQuery) loadJobOffers(ctx context.Context, query *JobOfferQuery, nodes []*Status, init func(*Status), assign func(*Status, *JobOffer)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(joboffer.FieldStatusID)
	}
	query.Where(predicate.JobOffer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(status.JobOffersColumn), fks...))
	}))

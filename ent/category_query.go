...
type CategoryQuery struct {
	...
	withChildCategories    *CategoryQuery
...
func (cq *CategoryQuery) QueryApplicantInterests() *ApplicantInterestQuery {
	query := (&ApplicantInterestClient{config: cq.config}).Query()
...
// QueryChildCategories chains the current query on the "child_categories" edge.
func (cq *CategoryQuery) QueryChildCategories() *CategoryQuery {
...
func (cq *CategoryQuery) QueryJobOfferCategories() *JobOfferCategoryQuery {
	query := (&JobOfferCategoryClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withApplicantInterests: cq.withApplicantInterests.Clone(),
		withChildCategories:    cq.withChildCategories.Clone(),
		withParentCategory:     cq.withParentCategory.Clone(),
		withJobOfferCategories: cq.withJobOfferCategories.Clone(),
...
func (cq *CategoryQuery) WithApplicantInterests(opts ...func(*ApplicantInterestQuery)) *CategoryQuery {
	query := (&ApplicantInterestClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withApplicantInterests = query
	return cq
}

// WithChildCategories tells the query-builder to eager-load the nodes that are connected to
// the "child_categories" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithChildCategories(opts ...func(*CategoryQuery)) *CategoryQuery {
	query := (&CategoryClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withChildCategories = query
	return cq
}
...
func (cq *CategoryQuery) WithJobOfferCategories(opts ...func(*JobOfferCategoryQuery)) *CategoryQuery {
	query := (&JobOfferCategoryClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withJobOfferCategories = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [4]bool{
			cq.withApplicantInterests != nil,
			cq.withChildCategories != nil,
			cq.withParentCategory != nil,
			cq.withJobOfferCategories != nil,
		}
	)
	...
	if query := cq.withApplicantInterests; query != nil {
		if err := cq.loadApplicantInterests(ctx, query, nodes,
			func(n *Category) { n.Edges.ApplicantInterests = []*ApplicantInterest{} },
			func(n *Category, e *ApplicantInterest) {
				n.Edges.ApplicantInterests = append(n.Edges.ApplicantInterests, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := cq.withChildCategories; query != nil {
		if err := cq.loadChildCategories(ctx, query, nodes,
			func(n *Category) { n.Edges.ChildCategories = []*Category{} },
			func(n *Category, e *Category) { n.Edges.ChildCategories = append(n.Edges.ChildCategories, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withParentCategory; query != nil {
		if err := cq.loadParentCategory(ctx, query, nodes, nil,
			func(n *Category, e *Category) { n.Edges.ParentCategory = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withJobOfferCategories; query != nil {
		if err := cq.loadJobOfferCategories(ctx, query, nodes,
			func(n *Category) { n.Edges.JobOfferCategories = []*JobOfferCategory{} },
			func(n *Category, e *JobOfferCategory) {
...
func (cq *CategoryQuery) loadApplicantInterests(ctx context.Context, query *ApplicantInterestQuery, nodes []*Category, init func(*Category), assign func(*Category, *ApplicantInterest)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(applicantinterest.FieldCategoryID)
	}
	query.Where(predicate.ApplicantInterest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ApplicantInterestsColumn), fks...))
	}))
...
func (cq *CategoryQuery) loadChildCategories(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Category(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ChildCategoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CategoryQuery) loadParentCategory(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
...
func (cq *CategoryQuery) loadJobOfferCategories(ctx context.Context, query *JobOfferCategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *JobOfferCategory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(joboffercategory.FieldCategoryID)
	}
	query.Where(predicate.JobOfferCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.JobOfferCategoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
